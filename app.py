...
# --- 3. CSS 設計 ---
custom_css = f"""
body {{ background: url('{base64_bg}') no-repeat center center fixed !important; background-size: cover !important; }}
.glass-panel {{ background: rgba(255, 255, 255, 0.9) !important; padding: 40px !important; border-radius: 30px !important; box-shadow: 0 10px 30px rgba(0,0,0,0.2) !important; }}
"""
...
with gr.Blocks(css=custom_css) as demo:
    ...
    with gr.Column(visible=False, elem_classes="glass-panel") as result_area:
        ...
        # 修正：將 outputs 的 [None, result_area] 改為對應的兩個有效元件 [main_area, result_area]
...
if __name__ == "__main__":
    # Render 會自動設定環境變數 PORT，若無則預設為 10000
    port = int(os.environ.get("PORT", 10000))
    # 綁定至 0.0.0.0 以確保 Render 可存取
